...
In a nutshell, GitOps offers a new pattern for deploying software that differs from the traditional CD pipeline approach that has dominated the industry for decades.
...
We’re very proud to announce that over 60 companies, 96 interested parties, and 34 co-authors have brought the best of DevOps into the [Open GitOps 1.0 principles](https://github.com/open-gitops/documents/releases/tag/v1.0.0).
...
As the [git history proves](https://github.com/open-gitops/documents/commits/v1.0.0), the wording of each of these principles was very carefully chosen.
While many of these principles should be familiar to DevOps professionals, many are surprised to learn that they still have work to do to implement GitOps fully.

Declarative is usually the easiest to understand.
...
Versioned and Immutable is often understood as “use git” but there is more to it than that.
...
Pulled Automatically means we have to have software agents constantly observing the desired state.
While triggers may exist to speed up that observation, a GitOps system shouldn’t exclusively rely on them.
Software agents need to be smart enough to constantly be aware of what the desired state should be.
The verb “pull” here is used very clearly in contrast to the way traditional CD pipelines function based on triggers that simply push.

Finally, Continuously Reconciling brings it all together.
...
[Get involved](https://opengitops.dev/get-involved)!
We’d love more participation to build GitOps Blueprints, GitOps Whitepapers, plan GitOps events, and help with the direction of the Open GitOps projects.
